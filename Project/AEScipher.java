  ...
  /**
   *inv_sbox:  Inverse Non-linear  substitution  table  used  in  several  byte  substitution
   *transformations  and  in  the  Key  Expansion  routine  to  perform  a  one  for  one
   *substitution  of  a  byte  value
   */
   ...
   *Nb  =  4;
   *"Nk"is  Number  of  32-bit  words  comprising  the  Cipher  Key.For  this  standard,Nk=4,6,or8.
   *Nk  =  KeyHex.length/4;
   ...
   *  @param  a is a key in hex
   *  @param  b is the plain text
   *  @return the matrix which is obtained after the X-OR
   */
  ...
  */
  ...
  for (j = 1; j < padLen; j++)
       padding[j] = (byte) 0;
  byte[] finalBytes = new byte[text.length + padding.length];
  System.arraycopy(text, 0, finalBytes, 0, text.length);
  System.arraycopy(padding, 0, finalBytes, text.length, padding.length);
  w  =  aesRoundKeyHexs(key);
  byte[] finalEncBlock = new byte[finalBytes.length];
  byte[] encBytes = new byte[padding.length];
  byte blockByte[] = new byte[16];
  int noOfBlocks = finalBytes.length / 16;
  for(int i=0,s=0;i<noOfBlocks;i++,s=s+16){
      blockByte = Arrays.copyOfRange(finalBytes,s,s+16);
      encBytes = encryptBloc(blockByte );
  System.arraycopy(encBytes,0,finalEncBlock,(0+(i*16)),16);
    }
  return  finalEncBlock;
  }
  ...
  */
  ...
  /**
   *
   * @param state  inverse of the byte substitution transformation,
   * in which the inverse S-box is applied to each byte of the State
   * @return the output which is obtained before substituting the s-box values.
   */
  ...
  /**
   *  @param  inStateHex  will perform the Shift Row operation to transform
   *  the hexadecimal input state matrix into output state
  ...
  /**
   *  @param  input - output from shift rows is taken as input
   *  i.e. in-state hex.
   *  @return the output which is obtained after the mixing columns.
   */
  ...
  /**
   *  @param  first takes the first row in the fixed state matrix to
   *  X-OR it with the first column in the current state matrix.
   *  @return the state matrix with all the columns mixed.
   */
  ...
  /**
   *@param in takes the cipher text as the input and performs all the
   * inverse operations for the steps which we have done while encrypting.
   * @return plain text.
   */
  ...
  /**
   * @param input Takes the cipher text obtained after encrypting
   * and then it checks for the one which is padded while encrypting
   * from the ending and delete all the zeros after one .
   * @return be able to return the same plaintext which is given while encrypting.
   */
  private static byte[] deletePadding(byte[] input) {
  int count = 0;
  byte checkFor = (byte) 0x80;
